...
from Dlg_XuatBieu11_156 import *
...
class DownloadDBR_Dlg(QDialog, Ui_Dialog_DownloadDBR):
	...
	def save_as(self):
		dir = QFileDialog.getSaveFileName(caption="q5PFES-Chọn thư mục lưu kết quả tải về",directory = "", filter= "Shape file(*.shp)", options = QFileDialog.Options() )[0]
...
class ChuanHoaDBR_Dlg(QDialog, Ui_Dialog_ChuanHoa):
	...
	def select_input_shape(self):
		...
		self.path_solution = str(QFileDialog.getOpenFileName(self, "q5PFES-Chọn lớp bản đồ diễn biến rừng", "", "Shapefile (*.shp)")[0])
	...
	def select_input_xls(self):
		self.path_solution = str(QFileDialog.getOpenFileName(self, "q5PFES-Chọn danh sách chủ rừng", "", "MS - Excel (*.xlsx);;MS - Excel (*.xls)")[0])
	...
	def select_output_shape(self):
		self.path_solution = str(QFileDialog.getSaveFileName(self, "q5PFES-Chọn thư mục lưu kết quả chuẩn hóa", "", "Shapefile (*.shp)")[0])
...
class XayDungCTDL_Dlg(QDialog, Ui_Dialog_XayDungCTDL):
	...
	def select_input_shape(self):	  
		self.path_solution = str(QFileDialog.getOpenFileName(self, "q5PFES-Chọn lớp bản đồ hiện trạng rừng", "", "Shapefile (*.shp)")[0])
	...
	def select_input_xls(self):
		self.path_solution = str(QFileDialog.getOpenFileName(self, "q5PFES-Chọn danh sách lưu vực", "", "MS - Excel (*.xlsx);;MS - Excel (*.xls)")[0])
		self.lineEdit_2.setText(self.path_solution)
		col_name = ['malv','tenlv']
		checkExcel(self,col_name)
		
	def select_output_shape(self):
		self.path_solution = str(QFileDialog.getSaveFileName(self, "q5PFES-Chọn thư mục chứa lớp bản đồ đầu ra", "", "Shapefile (*.shp)")[0])
...
class CapNhatVCT_Dlg(QDialog, Ui_Dialog_CapNhatVCT):
	...
	def select_input_shape(self):	  
		self.path_solution = str(QFileDialog.getOpenFileName(self, "q5PFES-Chọn lớp bản đồ đầu vào", "", "Shapefile (*.shp)")[0])
		self.lineEdit.setText(self.path_solution)
	...
	def select_input_luuvuc(self):
		self.path_solution = str(QFileDialog.getOpenFileName(self, "q5PFES-Chọn lớp ranh giới lưu vực", "", "Shapefile (*.shp)")[0])
		self.lineEdit_2.setText(self.path_solution)
...
class CapNhatNSK_Dlg(QDialog, Ui_Dialog_CapNhatNSK):
	...
	def __init__(self, iface):
		...
		self.setupUi(self)
		self.Inpath.setText('')
		self.btn_Inp.clicked.connect(self.select_input_shape)
		self.unPayment.setText('')
		self.btn_nsk.clicked.connect(self.select_input_nsk)
		self.Outpath.setText('')											 
		self.btnOut.clicked.connect(self.select_output_shape)
		self.buttonBox.accepted.connect(self.checkRun)
	...
	def checkRun(self):
		if self.Inpath.text() != '' and self.unPayment.text() != '' and self.Outpath.text() != '':
			self.run()
		else:
			self.iface.messageBar().pushMessage("Chưa chọn đủ dữ liệu", level=Qgis.Warning, duration=5)
	...
	def select_input_nsk(self):
		self.path_solution = str(QFileDialog.getOpenFileName(self, "q5PFES-Chọn lớp ranh giới khu vực đã có nguồn đầu tư khác", "", "Shapefile (*.shp)")[0])
		self.unPayment.setText(self.path_solution)

		err =0
		if self.path_solution != '':
			inputLV = self.unPayment.text()
			driver = ogr.GetDriverByName("ESRI Shapefile")
			dataSource = driver.Open(inputLV, 0)
			layer = dataSource.GetLayer()
			count_row = layer.GetFeatureCount()
	...
	def run(self):
		try:
			...
			for n in range(1,k):
				i=i+1
				if n == 1:
					fixgeometry(inNsk, fixLV_tempo)
				elif n == 2:
					buffer(fixLV_tempo,buffer_tempo)
				else:
					update_unpaymentArea(inpath,buffer_tempo,outpath)
					QgsProject.instance().removeAllMapLayers()
					shp = QgsVectorLayer(outpath, fname, 'ogr')
					layer = QgsProject.instance().addMapLayer(shp)
...
class XuatBieuNhom1_Dlg(QDialog, Ui_Dialog_XuatBieuNhom1):
	...
	def select_output(self):
		self.path_solution = str(QFileDialog.getExistingDirectory(self, "q5PFES-Chọn thư mục lưu kết quả"))
...
class XuatBieu11_156_Dlg(QDialog, Ui_Dialog_XuatBieu11_156):
	def __init__(self, iface):
		QDialog.__init__(self)
		self.iface = iface
		self.setupUi(self)
	...
	def run(self):
		try:		
			...
			export = outpath + '/bieu1.xlsx'
			tinh_out = outpath + '/TenTinh.xlsx'
			huyen_out = outpath + '/TenHuyen.xlsx'
			xa_out = outpath + '/TenXa.xlsx'
			copy = str(Path(__file__).parent.absolute()) + '/data/Mau11_156.xlsm'
			copy_js = str(Path(__file__).parent.absolute()) + '/data/dsluuvuc.json'
			paste = outpath + '/' + str(maxa) +'_'+ convert_TVKD(xa)+'.xlsm'
			paste_js = outpath + '/dsluuvuc.json'
			run = 'start excel.exe '
			...
			for n in range(1,6):
				i=i+1
				if n==1:
					report_comumune_11_156(tempo,maxa,export)
				elif n==2:
					tinh_export(tempo,maxa,tinh_out)
				elif n==3:
					huyen_export(tempo,maxa,huyen_out)
				elif n==4:
					xa_export(tempo,maxa,xa_out)
				else:
					shutil.copy2(copy, paste)
	...
	def close_form(self):
		...
		if os.path.exists(pathout + '/TenXa.xlsx'):
			os.remove(pathout + '/TenXa.xlsx')
		if os.path.exists(pathout + '/TenHuyen.xlsx'):
			os.remove(pathout + '/TenHuyen.xlsx')
		if os.path.exists(pathout + '/TenTinh.xlsx'):
			os.remove(pathout + '/TenTinh.xlsx')
		if os.path.exists(pathout + '/bieu1.xlsx'):
			os.remove(pathout + '/bieu1.xlsx')
